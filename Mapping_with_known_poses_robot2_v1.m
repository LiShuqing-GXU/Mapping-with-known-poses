...
X2=cell2mat(X2);
clearvars -except  dt2 X2 scans2 
clc;
...
prob_map2=zeros(CellNum_x,CellNum_y,Num_Of_Sim_steps);
...
for t=1:Num_Of_Sim_steps
    ...
    robot_x=X2(t,1); robot_y=X2(t,2); robot_theta=X2(t,3);%+pi/2;%+3*pi/8+pi; %+X1(t+1,3);
    scans_cart = scan2cart(scans2{t,1}, robot_theta );
...
%% Save and Make Video
% video = VideoWriter(['Known_poses_mapping_robot2_',date,'.mp4']);
% video.FrameRate = 12;
% open(video)
% writeVideo(video,F);
% close(video)
